...
export default function ContactScreen({
  ...
  // Always use the correct Indian number for WhatsApp
  // Strip all non-digits, then ensure 91 prefix → always 918958999299
  const cleanPhone = phone.replace(/[^0-9]/g, "");
  const whatsappNumber =
    cleanPhone.startsWith("91") && cleanPhone.length >= 12
      ? cleanPhone.slice(0, 12)
      : `91${cleanPhone.replace(/^0+/, "").slice(0, 10)}`;
  ...
  return (
    ...
    >
      ...
      >
        {/* Phone number display — flat section */}
        <div
        ...
        {/* Info section — flat, subtle background */}
        <div
          className="px-5 py-4 text-center"
          style={{ background: "oklch(0.93 0.05 147)" }}
        >
